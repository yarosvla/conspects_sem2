...
\section{Метрическая задача коммивояжёра. Алгоритм двойного дерева и алгоритм Кристофидеса--Сердюкова для её приближённого решения}
...
\begin{Example}{Примеры метрических весов}{}
	\begin{enumerate}
		...
		\item $c\colon E \to \{1,2\}$ --- весовая функция, принимающая только значения $1$ и $2$. Это также удовлетворяет неравенству треугольника ($1+1 \geq 1$, $1+2 \geq 2$, $2+2 \geq 2$).
	\end{enumerate}
	
	\begin{center}
		\begin{tikzpicture}[
			every node/.style={circle, fill=stmtFrame!50, draw=stmtFrame,
				inner sep=2pt, minimum size=5pt},
			edge/.style={thick}
			]
			\node (u) at (0,0) {};
			\node (v) at (1.5,1.2) {};
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[
		every node/.style={circle, draw=stmtFrame, fill=stmtFrame!20,
			inner sep=2pt, minimum size=6pt},
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Алгоритм двойного остова: исходное дерево $T$ (тёмные рёбра) и его задвоенная копия (цветные рёбра). После удвоения все вершины имеют чётную степень, что гарантирует существование Эйлерова цикла.}
	\label{fig:double-tree}
\end{figure}

\begin{Theorem}{О 2-приближении алгоритма двойного остова}{}
	Пусть $H^*$ --- минимальный по весу гамильтонов цикл и $T$ --- минимальное остовное дерево. Тогда построенный алгоритмом гамильтонов цикл $\widetilde{H}$ удовлетворяет:
	\[
...
\begin{proof}
	\begin{enumerate}
		...
		\item \emph{Срезание вершин не увеличивает вес.} При построении $\widetilde{H}$, если вершина $u_i$ уже встречалась, <<перепрыгиваем>> через неё: вместо рёбер $(u_{i-1},u_i)$ и $(u_i,u_{i+1})$ используем ребро $(u_{i-1},u_{i+1})$. По неравенству треугольника:
		\[
...
Алгоритм Кристофидеса--Сердюкова улучшает коэффициент приближения с $2$ до $\tfrac{3}{2}$, заменяя дублирование всех рёбер дерева минимальным паросочетанием на вершинах нечётной степени.
...
\begin{enumerate}
	...
	\item Построить минимальное по весу совершенное паросочетание $M$ на вершинах $V_0$ (алгоритм Эдмондса работает за полиномиальное время).
	\item Добавить рёбра паросочетания $M$ к дереву $T$; получим мультиграф $T' = T \cup M$, в котором все вершины имеют чётную степень.
...
\begin{figure}[H]
	...
	\caption{Пример работы алгоритма Кристофидеса--Сердюкова: чёрные рёбра --- минимальное остовное дерево $T$, красные рёбра --- минимальное совершенное паросочетание $M$ на вершинах нечётной степени $V_0$. Мультиграф $T' = T \cup M$ имеет все вершины чётной степени.}
...
\begin{proof}
	Вес $c(\widetilde{H})$ не превышает суммарного веса Эйлерова цикла в $T'$ (срезание не увеличивает вес по неравенству треугольника):
	\[
	c(\widetilde{H}) \leq c(T) + c(M),
	\]
	где $c(M) = \sum_{e \in M} c(e)$ --- вес минимального паросочетания на $V_0$.
	
	\emph{Шаг 1: $c(T) \leq c(H^*)$.} Граф $H^* \setminus \{e\}$ для любого $e \in E(H^*)$ является остовным деревом, поэтому $c(T) \leq c(H^*)$.
	
	\emph{Шаг 2: $c(M) \leq \tfrac{c(H^*)}{2}$.} Пронумеруем вершины $V_0 = \{v_1, v_2, \ldots, v_{2r}\}$ в порядке их встречаемости при обходе оптимального цикла $H^*$. Рассмотрим два паросочетания на $V_0$:
	\[
	M_1 = \{v_1v_2,\; v_3v_4,\; \ldots,\; v_{2r-1}v_{2r}\}, \qquad
	M_2 = \{v_2v_3,\; v_4v_5,\; \ldots,\; v_{2r}v_1\}.
	\]
	По обобщённому неравенству треугольника (применённому к путям вдоль $H^*$ между соседними вершинами $V_0$):
	\[
	c(M_1) + c(M_2) \leq c(H^*).
	\]
	Следовательно, $\min(c(M_1),c(M_2)) \leq \tfrac{c(H^*)}{2}$. Минимальное паросочетание не хуже наилучшего из $M_1, M_2$:
	\[
...
Ключевую идею оценки минимального паросочетания на $V_0$ иллюстрирует следующая схема: цикл $H^*$ разбивается на два паросочетания $M_1$ и $M_2$, и их суммарный вес не превосходит $c(H^*)$.

\providecommand{\cloudcycle}[4]{%
	\draw[thick]
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[
		dot/.style={circle, draw, fill=white, thick, inner sep=0pt, minimum size=7pt},
		m1/.style={thmFrame, very thick},
		m2/.style={exFrame, very thick},
		vlabel/.style={draw=none, fill=none, font=\small},
		cappp/.style={draw=none, fill=none, font=\small, align=center, text width=3.6cm}
		]
		...
		\begin{scope}[xshift=0cm]
			...
			\node[cappp] at (1.25,-1.25) {$H^*$, $V_0=\{v_1,v_2,v_3,v_4\}$};
		\end{scope}
		...
		\begin{scope}[xshift=4.9cm]
			...
			\draw[m1] (B1)--(B2);
			\draw[m1] (B3)--(B4);
			\node[cappp, text=thmFrame] at (1.25,-1.25) {$M_1=\{v_1v_2,\;v_3v_4\}$};
		\end{scope}
		...
		\begin{scope}[xshift=9.8cm]
			...
			\draw[m2] (C2)--(C3);
			\draw[m2] (C4)--(C1);
			\node[cappp, text=exFrame] at (1.25,-1.25) {$M_2=\{v_2v_3,\;v_4v_1\}$};
		\end{scope}
		
	\end{tikzpicture}
	\caption{Разбиение оптимального цикла $H^*$ на два паросочетания $M_1$ и $M_2$. Их суммарный вес $c(M_1)+c(M_2) \leq c(H^*)$, откуда минимальное паросочетание удовлетворяет $c(M) \leq \tfrac{c(H^*)}{2}$.}
	\label{fig:two-matchings}
\end{figure}
