...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For{$d=0$ \textbf{to} $\log_2 n - 1$} \Comment{для фиксированного $d$ все операции внутреннего цикла по $k$ выполняются параллельно}
		\For{$k=0$ \textbf{to} $n-1$ \textbf{step} $2^{d+1}$}
		\State $x[k+2^{d+1}-1] = x[k+2^{d+1}-1] + x[k+2^{d}-1]$
		\EndFor
		\EndFor
		\Comment{Downsweep: добавляем префикс предыдущего блока к левому подблоку}
		\For{$d=\log_2 n - 1$ \textbf{downto} $0$} \Comment{для фиксированного $d$ все операции внутреннего цикла по $k$ выполняются параллельно}
		\For{$k=0$ \textbf{to} $n-1$ \textbf{step} $2^{d+1}$}
		\If{$k>0$}
		\State $x[k+2^{d}-1] = x[k+2^{d}-1] + x[k-1]$
		\EndIf
		\EndFor
		\EndFor
	\end{algorithmic}
...
\begin{Example}{Сортировка подсчётом (Counting sort) в PRAM}{}
	...
	Каждый процессор обрабатывает блок длины $m=\frac{n}{p}$.
	...
	\begin{itemize}
		...
		\qquad m=\frac{n}{p}.
		\]
		Это занимает $O(\frac{n}{p})$ времени.
		
		\item Далее соберём все частоты в один массив длины $(U+1)p$ в порядке
		\[
		C[xp+i]=c_i(x),\qquad x=0,\dots,U,\ \ i=0,\dots,p-1,
		\]
		то есть
		\[
		C = \bigl(c_0(0),c_1(0),\dots,c_{p-1}(0),\ c_0(1),\dots,c_{p-1}(1),\ \dots,\ c_{p-1}(U)\bigr).
		\]
		Посчитаем \texttt{scan} (префиксные суммы) по $C$ и получим массив $P$.
		
		\item После этого процессор $i$ знает, сколько элементов значения $x$
		находится у процессоров с номерами $\le i$ (это $P[xp+i]$), а значит знает и
		\textbf{диапазон позиций} в выходном массиве для своих элементов значения $x$:
		\[
		\text{элементы }x\text{ из блока }i\text{ кладём в } 
		\bigl[\,P[xp+i]-c_i(x),\ P[xp+i)\,\bigr)
		\]
		И просто выписываем элементы в эти позиции.
		
	\end{itemize}
	
	\paragraph{Время.}
	Локальный подсчёт частот занимает $O(n/p)$.
	\texttt{scan} по массиву длины $(U+1)p$ занимает $O(\log((U+1)p))=O(\log p)$, так как $U=O(1)$.
	Итого время:
	\[
	O\!\left(\frac{n}{p}+\log p\right).
	\]
