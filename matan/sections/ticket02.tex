...
\begin{remark}
    ...
    Существует "обратное" неравенство Минковсокго, которое доказаывается точно также:
\end{remark}
...
\begin{proof}
    ...
    Построим последовательность $\{l(\Gamma'_k)\}$, предел который будет равен $l(\Gamma) = \sup(l(\Gamma'))$.
    ...
    Чтобы дальнейшие выкладки были немного чище, опустим символы диффернциала и подстановки переменной.

    Оценим разность 

    \[
    \int_a^b{\sqrt{\sum_{n=1}^d{|\vphi_n'|^2}}} - l(\Gamma_k') =
    \]
    \[
    = \int_a^b{\sqrt{\sum_{n=1}^d{|\vphi_n'|^2}}} - 
    \sum_{j=0}^{N_k-1}\sqrt{\sum_{n=1}^d{(\vphi_n(t_{k, j+1}) - \vphi_n(t_{k, j}))^2}} = 
    \]

    \[
    = \sum_{j=0}^{N_k-1} \int_{t_{k, j}}^{t_{k,j+1}}{\sqrt{\sum_{n=1}^d{|\vphi_n'|^2}}} -
